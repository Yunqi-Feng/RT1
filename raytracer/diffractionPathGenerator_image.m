function [output, multipath, tmp_plane, plane_mat,diffraction_loss_dB] = diffractionPathGenerator_image(Tx, Rx, wedges, CADOutput, ...
vtx, vrx, frequency,LOS_output,pseudo_LOS_output,polarization,Jones,Ptx, antenna, enablePhase,...
materialLibrary, mat_confg, IndoorSwitch, tmp_plane, plane_mat,reflecting_plane_eq)
...
%   multipath:   The coordinates of the diffracted paths for visualization.
    ...
    for i = 1:size(wedges, 1)
        ...
        Tx_dir = -(diffraction_point - Tx);
        a = reflecting_plane_eq(1);
        b = reflecting_plane_eq(2);
        c = reflecting_plane_eq(3);
        d = reflecting_plane_eq(4);
        x0 = Tx(1);
        y0 = Tx(2);
        z0 = Tx(3);
        vx = Tx_dir(1);
        vy = Tx_dir(2);
        vz = Tx_dir(3);
        t = (d - (a * x0 + b * y0 + c * z0)) / (a * vx + b * vy + c * vz);

        Tx_tmp = [x0 + t * vx, y0 + t * vy, z0 + t * vz];
        is_path_to_edge_clear = verifyPath(Tx_tmp, diffraction_point, diffraction_point - Tx, [0,0,0,0], [0,0,0,0], CADOutput, 2, false);
