function [x_cen,y_cen,z_cen] = mysphere(dis,in,ou,d,mean,std,NodeLocTx,NodeLocRx,scatterer_height_max)
...
z2=NodeLocRx(3);
if mean<in || mean>ou
    error('inappropriate configuration, the mean of a sphere should be within the inner radius and outer radius')
end
if in<0 || ou<0
...
if dis==0%uniform
    ...
    phi=2*pi*rand;
    theta=-pi+2*pi*rand;
    x_cen=x2+r*sin(theta)*cos(phi);
    y_cen=y2+r*sin(theta)*sin(phi);
    z_cen=min(scatterer_height_max,z2+r*cos(theta));
